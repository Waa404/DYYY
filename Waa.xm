...
- (void)layoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"WaaEnableHomeFullScreen"]) {
        ...
        BOOL shouldDelayShow = NO;

        for (UIView *subview in self.subviews) {
            if ([subview isKindOfClass:NSClassFromString(@"AWEAwemeDetailTableViewCell")]) {
                UITableViewCell *cell = (UITableViewCell *)subview;

                CGFloat originalHeight = cell.frame.size.height;
                if (originalHeight > 0) {
                    kLatestCellHeight = originalHeight;
                    if (kOriginalCellHeight == 0 || originalHeight < kOriginalCellHeight) {
                        kOriginalCellHeight = originalHeight;
                    }
                ...
                NSInteger row = indexPath ? indexPath.row : 0;

                NSInteger offsetCount = 0;
                if (row >= 6) {
                    offsetCount = ((row - 6) / 11) + 1;
                }
                ...
                if (ViewJudgment) {
                    frame.origin.y = fullHeight * (row - offsetCount);
                }
...
- (void)viewDidLayoutSubviews {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"WaaEnableHomeFullScreen"]) {
        if (ViewJudgment) {
            ...
            frame.size.height = kOriginalCellHeight;
